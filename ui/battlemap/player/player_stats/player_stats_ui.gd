...
func _on_player_health_changed(_current_health: int):
	_animate_progress_bar(health_progress_bar, _current_health)
	player_health_label.text = str(_current_health) + "/" + str(100)
...
func _animate_progress_bar(_progress_bar: ProgressBar, value: int):
	...
	tween.tween_property(_progress_bar, "value", value, 0.2)
	# Brief highlight effect
	var highlight_tween = create_tween()
	highlight_tween.tween_property(_progress_bar, "modulate", Color(1.2, 1.2, 1.5, 1), 0.1)
	highlight_tween.tween_property(_progress_bar, "modulate", Color.WHITE, 0.2)
