...
## Available world positions indexed by distance.
## Kept for API compatibility; not used by the new layer-based generation.
...
# ──────────────────────────────────────────────
#  Distance / spatial helpers (kept for save-load and debug use)
# ──────────────────────────────────────────────
...
func _table_position_distance(
	...
	return distance_between_two_points(
		_table_node_a.table_position,
		_table_node_b.table_position
	)
...
func distance_between_two_points(
	...
	var y1 = _point_a.y
	var y2 = _point_b.y
	return roundi(sqrt(pow(x2 - x1, 2) + pow(y2 - y1, 2)))

## Returns a random available position, or (-1, -1) if none available.
## Retained for API compatibility; not called by the layered generator.
func get_random_position(
	_min_distance_to_root: int = 0,
	_max_distance_to_root: int = 101
) -> Vector2:
	if _available_world_table_positions.is_empty():
		return Vector2(-1, -1)
...
func has_table_pos_in_distance(_distance: int) -> bool:
	return _available_world_table_positions.has(_distance) \
		and not _available_world_table_positions[_distance].is_empty()

## Returns the root node for the given position / distance.
func get_origin_node(_node_pos: Vector2, _distance: int = 0) -> GenericWorldNode:
...
func calculate_positions_in_radius(
	...
	return center_node.global_position + offset

## Hex-grid screen-position helper (kept for any external callers).
func calculate_positions_in_radius_hex(
	...
	var offset_y   = table_offset.y * hex_height * 0.90 \
		+ (int(table_position.x) % 2) * hex_height * 0.5 \
		- (int(center_table_position.x) % 2) * hex_height * 0.5
	return center_node.global_position + Vector2(offset_x, offset_y)
...
func store_adjacent_table_positions(
	...
	var node_distance_to_center: int = self.distance_between_two_points(
		_table_center_point, center_position
	)
	var radius: int = 1
	for radius_x in range(-radius, radius + 1):
		for radius_y in range(-radius, radius + 1):
			...
			var _distance_to_root: int = self.distance_between_two_points(
				_table_position, _table_center_point
			)
...
func is_position_already_added(_table_pos: Vector2) -> bool:
	return _added_table_positions.has(_table_pos)

func block_table_depth(_depth: int) -> void:
	if not _available_world_table_positions.has(_depth):
		return
	_available_world_table_positions.erase(_depth)

func block_adjancent_table_positions(_node_table_pos: Vector2) -> void:
	var radius: int = 1
	for radius_x in range(-radius, radius + 1):
		for radius_y in range(-radius, radius + 1):
			var _table_position: Vector2 = Vector2(
			...
			var _distance: int = self.distance_between_two_points(
				_table_center_point, _table_position
			)
...
func _show_available_pos() -> void:
	...
	for _distance: int in _available_world_table_positions.keys():
		print("DEBUG: _distance: ", _distance, " pos: ",
			_available_world_table_positions[_distance].keys())
